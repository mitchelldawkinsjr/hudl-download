(async function () {
  ...
  streams.forEach((s) => {
    ...
    const label = s.type.toUpperCase() + ' stream';
    row.innerHTML = `
      <div>${label}</div>
      <div class="url">${s.url}</div>
      <button data-url="${s.url}">Download</button>
    `;
  ...
  function trackJob(jobId) {
    ...
    chrome.runtime.onMessage.addListener(function listener(msg) {
      ...
      if (msg.status === 'fetching-manifest') {
        ...
        statusEl.textContent = `Done. Saved to Downloads/${msg.folder}. Run: node downloader/remux.js "<path-to-folder>"`;
